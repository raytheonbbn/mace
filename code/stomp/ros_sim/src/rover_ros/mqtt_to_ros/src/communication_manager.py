...
class CommunicationManager():
  ...
  def __init__(self, host="localhost", port=1883):
    ...
    self.client.username_pw_set("white_force", "maceRef!")
    self.client.tls_set(ca_certs="/etc/mosquitto/ca_certificates/ca.crt", tls_version=ssl.PROTOCOL_TLSv1_2)
  ...
  def message_callback(self, client, userdata, message):
    ...
    # If its a quad rotor
    elif "Quad" in data["uid"].lower() or "Rover" in data["uid"].lower():
      # If we have not seen this target before, create the it
      if data["uid"] not in self.target_information:
