...
run_in_background()
{
   ...
   python3 SimPayloadManager.py --num_quad_payloads=${numSolos} --num_rover_payloads=${numRovers} & #>/dev/null 2>&1 &
...
run_with_logs()
{
   ...
   # Start the target as a background process 
   python3 SimPayloadManager.py --num_quad_payloads=${numSolos} --num_rover_payloads=${numRovers} --log & #>/dev/null 2>&1 &
   echo "Starting SimPayloadManager on pid $!"
   echo "$!" >> ${PID_FILE}
}
